...
//! pure-data `state` modules, and the `command` side-effect enum. Per the
//! development spec, all app state is mutated only through `App::update`,
//! never from tasks or render code.
